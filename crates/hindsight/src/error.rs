...
//!
//! Page failures render a small branded HTML error page; the JSON API paths still get a
//! sensible status code. 401s additionally carry `WWW-Authenticate`. Keeping one enum mirrors
//! the keystone/inkwell error seam.
...
#[derive(Debug, Error)]
pub enum AppError {
    /// Malformed/incomplete form input (empty title, bad window, etc.).
...
/// Store failures collapse to a 500 (no user-recoverable shape exists for incident/note I/O).
