...
//!
//! Hindsight does NO login of its own. It sits behind a Sluice `auth=sso` route, where the
//! gateway runs the OIDC browser login against Keystone, STRIPS any inbound `X-Auth-*`, and
//! injects the verified `X-Auth-Subject` / `X-Auth-Email` / `X-Auth-Scope`. Because Hindsight
//! is internal-only (never publicly reachable), it TRUSTS those headers as the authenticated
//! operator — the incident `created_by` / note `author_sub` are ALWAYS taken from the gateway
//! headers, never from a client-supplied field.
//!
//! State-changing POSTs (open an incident / add a note) are double-submit CSRF protected: a
//! random token lives in a JS-readable `__Host-csrf` cookie AND in a hidden form field; the
//! POST is accepted only when the two match. The token is minted once and REUSED across page
//! renders (so it stays stable across tabs for the cookie lifetime).
...
/// The injected authorization scope, if any (informational; Sluice already gates the route).
...
pub fn display_email(headers: &HeaderMap) -> String {
    email(headers).unwrap_or_else(|| "operator".to_string())
}
...
pub fn require_operator(headers: &HeaderMap) -> Result<(String, String), AppError> {
    ...
    let email = email(headers).unwrap_or_default();
    Ok((sub, email))
}
